...
function openDecisionDialog(row?: EffectivePricingReportRow) {
	const currentRow = rows.value[rows.value.length - 1]
  Object.assign(decisionForm, {
    model: row?.upstream_model || modelFilter.value, upstream_model: row?.upstream_model || modelFilter.value,
    protocol: row?.protocol || protocolFilter.value, current_provider_account_id: currentRow?.provider_account_id || '',
    candidate_provider_account_id: row?.provider_account_id || rows.value[0]?.provider_account_id || ''
  })
...
function openProbeDialog(row?: EffectivePricingReportRow) {
  ...
  Object.assign(probeForm, {
    provider_account_id: source?.provider_account_id || accountOptions.value[0]?.id || '',
    upstream_model: source?.upstream_model || modelFilter.value,
...
<template>
  <main class="content effective-pricing-page">
    ...
    <template v-if="activeTab === 'pricing'">
      <section class="panel effective-panel">
        ...
        <div class="panel-body table-scroll ep-table"><table class="data-table"><thead><tr><th>{{ t('effectivePricing.supplierAccount') }}</th><th>{{ t('effectivePricing.quoted') }}</th><th>{{ t('effectivePricing.billed') }}</th><th>{{ t('effectivePricing.effective') }}</th><th>{{ t('effectivePricing.effectiveCost') }}</th><th>{{ t('effectivePricing.cacheHit') }}</th><th>{{ t('effectivePricing.coverage') }}</th><th>{{ t('effectivePricing.confidence') }}</th><th>{{ t('common.actions') }}</th></tr></thead><tbody>
          ...
          <tr v-if="!rows.length"><td colspan="9" class="empty-cell">{{ loading ? t('common.loading') : t('effectivePricing.empty') }}</td></tr>
        </tbody></table></div>
      </section>
    </template>
    ...
    <template v-else-if="activeTab === 'switches'">
      <div class="switch-head"><div><h2>{{ t('effectivePricing.switchCenter') }}</h2><p>{{ t('effectivePricing.switchHelp') }}</p></div><button class="button" type="button" :disabled="rows.length < 2" @click="openDecisionDialog()"><Plus :size="16" />{{ t('effectivePricing.newEvaluation') }}</button></div>
      <section class="decision-grid"><article v-for="decision in decisions" :key="decision.id" class="panel decision-card"><header><div><span class="pill" :class="statusClass(decision.status)">{{ decision.status }}</span><h3>{{ decision.model }}</h3><p>{{ decision.protocol }} · {{ decision.id }}</p></div><strong>{{ formatPercent(decision.cost_improvement) }}</strong></header><div class="decision-compare"><div><small>{{ t('effectivePricing.current') }}</small><strong>{{ accountName(decision.current_provider_account_id) }}</strong><span>{{ formatMoneyMicros(decision.current_cost_micros_per_1m) }}</span></div><div><small>{{ t('effectivePricing.candidate') }}</small><strong>{{ accountName(decision.candidate_provider_account_id) }}</strong><span>{{ formatMoneyMicros(decision.candidate_cost_micros_per_1m) }}</span></div></div><p v-if="decision.reason_codes.length" class="decision-reasons">{{ decision.reason_codes.join(' · ') }}</p><footer class="row-actions"><button v-if="decision.status === 'recommended'" class="button" type="button" @click="decisionAction(decision, 'approve_canary')">{{ t('effectivePricing.startCanary') }}</button><button v-if="decision.status === 'canary'" class="button" type="button" @click="decisionAction(decision, 'activate')">{{ t('effectivePricing.activate') }}</button><button v-if="['canary','active','degraded'].includes(decision.status)" class="button secondary" type="button" @click="decisionAction(decision, 'rollback')">{{ t('effectivePricing.rollback') }}</button></footer></article><div v-if="!decisions.length" class="panel empty-cell">{{ t('effectivePricing.noDecisions') }}</div></section>
    </template>
    ...
    <div v-if="dialog" class="modal-backdrop"><form class="modal-card effective-dialog" @submit.prevent="saveDialog"><header class="modal-header"><div><h2>{{ t(`effectivePricing.dialogs.${dialog}`) }}</h2><p>{{ t(`effectivePricing.dialogHelp.${dialog}`) }}</p></div><button class="icon-button" type="button" @click="dialog = null"><X :size="18" /></button></header><div class="modal-body form-grid">
      ...
      <template v-else><div class="field"><label>{{ t('effectivePricing.routeModel') }}</label><input v-model="decisionForm.model" required /></div><div class="field"><label>{{ t('effectivePricing.protocol') }}</label><input v-model="decisionForm.protocol" required /></div><div class="field"><label>{{ t('effectivePricing.current') }}</label><select v-model="decisionForm.current_provider_account_id" required><option v-for="row in rows" :key="row.provider_account_id" :value="row.provider_account_id">{{ row.provider_account_name || row.provider_account_id }}</option></select></div><div class="field"><label>{{ t('effectivePricing.candidate') }}</label><select v-model="decisionForm.candidate_provider_account_id" required><option v-for="row in rows" :key="row.provider_account_id" :value="row.provider_account_id">{{ row.provider_account_name || row.provider_account_id }}</option></select></div></template>
    </div><footer class="modal-footer"><button class="button secondary" type="button" @click="dialog = null">{{ t('common.cancel') }}</button><button class="button" type="submit" :disabled="saving || (dialog === 'probe' && !probeBudgetConfirmed)"><Save :size="17" />{{ saving ? t('common.saving') : t('common.save') }}</button></footer></form></div>
  </main>
</template>

<style scoped>
.effective-pricing-page { display: grid; gap: 16px; }
